...
### The four colour families — keep them distinguishable

This is the constraint most easily broken, and it took several passes to get
right.

**Separation comes from shape, placement and lightness. Hue is only required
not to be identical.** An earlier draft of this section asked for four families
with no shared hexes and no accent "near" an outcome, and that cannot be built:
six reserved status hues with workable buffers leave only two usable bands in
the whole 360° circle, so six user-settable accents and a four-family status
palette do not fit. Demanding hue separation everywhere would either cost the
accent picker most of its range or push the status colours into each other.

What actually keeps these apart is that they are different *things* in
different *places*: outcome is a bar that is filled or outlined, availability is
a dot in an avatar's bottom-right corner, game state is a square on a game card,
sport identity tints a divider. Two colours 10° apart in different shapes in
different places have never been confused. Two colours 1° apart in the same
size mark, 40px from each other, always will be — that was Gold against
`--status-questionable`, and it is the kind of collision this rule is for.

So the test is: **is this colour identical to one in another family, in a place
a reader could mistake one for the other?** Not: is it near one.
...
2. **Availability** — literal hexes by design, never the accent:
   ...
   Availability shares `#3ecf8e` and `#ef5b5b` with outcome by default. That is
   allowed and is not an aliasing mistake: the two are *defined independently*,
   so when a user changes the outcome palette (see Settings) availability does
   not move with it. What must never happen is `--status-available: var(--pos)`.

   That last line is the one somebody will "tidy" — two tokens holding the same
   hex look like duplication worth collapsing. They are not. **Outcome colours
   are user-settable** (`display.statusPalette`, `posColor`, `negColor`), so
   aliasing availability onto `--pos`/`--neg` would hand a palette preference
   the power to silently retint injury status: pick the teal/magenta pair and
   every available player goes teal, every ruled-out player magenta. Health is
   not a display preference. The duplication is what keeps a cosmetic choice
   from rewriting a medical fact.
3. **Game state** (Games, Mobile) — **one hue per pair, split by fill.** The
   active half of a pair is filled, the paused or not-yet half is hollow, which
   covers the nine `GAME_STATUS` values in `lib/gamesData.js` with four colours
   and one rule:

   | Token | Filled | Hollow |
   |---|---|---|
   | `--state-live` `#22c9d6` | LIVE | HALFTIME, INTERMISSION |
   | `--state-pre` `#e8ecf2` | STARTING_SOON | UPCOMING |
   | `--state-delayed` `#9b8cf0` | DELAYED | SUSPENDED |
   | `--state-final` `#8b98ab` | FINAL | POSTPONED |

   Rendered as **squares**, so shape separates them from outcomes. Do not split
   a pair across two hues — the pairing is what keeps the family to four.

   Pre-game was lapis in the mocks and final was white; both moved. Lapis
   `#8fa6ff` is the default accent's own hue, so an upcoming game and a selected
   control were the same colour — moving pre-game to white removes that at the
   source rather than managing it, and takes lapis out of this family entirely.
   Final moving down to dim is also the better semantic: a finished game is the
   least urgent thing on a slate and the mocks rendered it the brightest.
   `--state-pre` and `--state-final` now share a hue and are told apart by
   lightness (0.93 against 0.61), which is the mechanism above working as
   intended.
...
**Accent presets** (`src/ColorWheel.jsx` `ACCENT_PRESETS`) — shipped: Lapis
`#3b5bdb`, Teal `#14b8a6`, Gold `#a87d2e`, Rust `#c2622d`, Violet `#8b5cf6`,
Magenta `#d6409f`.

Green `#22c55e` and Red `#ef4444` were dropped: Red was 0.0° from `--neg` and
Green 11° from `--pos`, i.e. identical and near-identical. Nobody lost a colour
— the accent persists as a hex, not a preset id, so an existing pick still
renders and only stops matching a swatch name.

Gold is `#a87d2e`, deepened from the mocks' `#c9a24a`, which was 0.5° and 0.03
lightness from `--status-questionable` — the same colour, and on a feed row the
availability dot and the accent-filled add button sit ~40px apart. It moved in
**lightness** (0.54 → 0.42), not hue, so it still reads as gold; shifting the
hue would have walked it into Rust.

The rest are near without being identical and are separated by shape and place:
Violet sits 9° from `--state-delayed`, Teal 11° from `--state-live`. That is
fine, and chasing it further is what the opening of this section warns against.
...
One bar per game, seated on a pedestal so a zero-value game is still a visible
mark. A **dashed rule** crosses at the prop line, so height reads as distance
from it.

Bar height is **not** the raw stat value on a zero baseline — that is failure
mode 2 below, and it is what the windowed axis exists to replace. The trade is
real and worth stating: a 3-hit game is not visibly three times a 1-hit game.
What it buys is that margin over and under the line stays legible whether the
line is 0.5 hits or 257.5 passing yards.
...
**Heights are comparable within a row, not across rows — by design.** Each row
windows its axis to its own games and its own line, so the same bar height means
different things in two different rows, and one row's tallest bar is not a
bigger number than another's. That is the point rather than a limitation: the
question a row answers is "how did this player do against *this* line", and an
axis shared across rows would have to span every market on screen, which is the
0–164 problem again at feed scale. Cross-row comparison is what the rate and
sample columns beside the graph are for.
...
**Slot count is a prop, not a constant.** Ten is the default; a surface that
draws a different window passes its own. The Board is the live example — its
cards slice to the last eight games, so they pass `slots={8}`. Left at ten, a
*full* board card labels two columns `no games yet` about a player who has more
games, which is not a thin sample but a false statement about the log. The grid
has to describe the window the surface actually draws. Windows longer than the
default (L20, L40) expand past it; the floor exists to stop four games filling
the row, not to cap how much log a graph can show.

### Sizes

`FORM_SIZES` in `src/FormGraph.jsx` is the source of truth for these pairs, not
this table — the table was transcribed from the mocks mid-iteration. Where the
two differ, the code is right and this table is stale. The Extras column is
still the useful part: it says what each context draws around the bars.
...
stepping in **whole numbers** (1 under 100, 5 at 100 and above) anchored on the
market line, clamped to the axis. A 0.5 step is what the half-value rule above
forbids: off an X.5 line it lands every other rung on a whole number, and it
puts the posted line itself out of reach (255.5 / 260.5 either side of 257.5).
`src/lib/altLines.js` holds the same rule as a correctness constraint — no
logged integer can sit on an X.5 line, so there are no pushes and `1 - rate` is
the exact Under rate. Every bar
...
opens the board, not when the page renders — dismissing on render would spend a
once-ever prompt on a bounce. Reachable afterwards from `How we count` and
Settings → Tutorial.

**`startPage` defaults to `feed`, which is what the app opens on today.** An
earlier draft here defaulted it to `board`, which conflated two separate
problems. The newcomer problem — *what is this, where do I start* — is what the
intro exists to solve, and it is solved once, on first run. `startPage` is a
preference for someone who already knows what they want. Defaulting it to a new
page would not help a newcomer, who sees the intro either way, while costing a
returning user the habit they already have. Adding the preference must move
nobody: it resolves to current behaviour until somebody deliberately changes it.

The general rule behind both: **neither the intro nor a new preference may
silently relocate an existing user.**
...
**`startPage` accepts every nav page except `landing`,** and the validation
lives next to the nav list in `PropLedger.jsx` (`PAGES` / `PAGE_IDS`) so the two
cannot drift. The intro is first-run state, not a destination: a `startPage` of
`landing` would put it back on every load *and* on every wordmark click, which
is precisely what `propPalaceTour.dismissed` exists to prevent. An unrecognised
stored value falls back to the default rather than routing nowhere. Do not
re-add `landing` to that set.

Writes to `propPalaceTour` **merge** rather than replace. Only `dismissed` is
read or written today; `step` and `completed` belong to the tutorial track,
which has not started, and a replacing write would erase its progress the first
time someone dismissed the intro.
