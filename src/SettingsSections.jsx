...
// Every settings control, in one place, because there are now two surfaces
// showing them: the full Settings page (SettingsPage.jsx, transcribed from
// `PropPalace Settings v2.dc.html`) and the cog's quick panel
// (SettingsModal.jsx).
//
// Moved here verbatim from the dialog rather than rewritten. The page renders
// the same four sections the dialog used to, so there is one definition of
// what a setting looks like instead of two that drift apart.
...
export function MoneyInput({ value, onChange, placeholder, prefix = "$" }) {
  return (
...
export function AccentTouchesExample() {
  return (
...
export function DisplaySection({ settings, isNarrow }) {
...
// one.
export function OutcomeColours({ settings, isNarrow }) {
...
export function BettingSection({ settings, sportsbooks }) {
...
export function LinkRow({ label, hint, action, href }) {
  return (
    <a
      href={href}
      target="_blank"
