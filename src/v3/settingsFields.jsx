...
// The Settings field set, shared by both v3 frames -- the phone's 3d and the
// desktop's 2h. They differ in where the tabs sit (a scrolling chip row, a
// 232px rail) and in nothing else, so the fields are written once.
//
...
// The desktop frame draws the ring inline instead of behind a sheet -- it has
// the room, and "Drag the ring, or pick one below" is only true when the ring
// is on the page. The phone frame keeps the 52px preview row and the sheet,
// because at 430px the ring and its presets are most of a screen.
function WheelInline({ value, onPick, onReset }) {
  const current = value || "#3b5bdb";
  const [angle, setAngle] = React.useState(() => angleOf(current) ?? DEFAULT_ANGLE);
  const drag = (e) => {
    e.preventDefault();
    ...
    const cy = box.top + box.height / 2;
    const at = (x, y) => Math.round((Math.atan2(y - cy, x - cx) * 180 / Math.PI + 450) % 360);
    const apply = (ev) => { const deg = at(ev.clientX, ev.clientY); setAngle(deg); onPick(hueHex(deg)); };
    apply(e);
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 18, padding: 15, border: "1px solid var(--line)", borderRadius: 12, background: "var(--surface-1)" }}>
      <span
        onPointerDown={drag}
        style={{ width: 108, height: 108, borderRadius: 999, flex: "0 0 auto", position: "relative", display: "block", cursor: "grab", touchAction: "none", background: "conic-gradient(#ef5b5b, #e8b13a, #3ecf8e, #2bb8a6, #3b5bdb, #8b5bd6, #ef5b5b)" }}
      >
        <span style={{ position: "absolute", top: "50%", left: "50%", width: 42, height: 42, marginTop: -21, marginLeft: -21, borderRadius: 999, background: current, border: "3px solid var(--bg)", display: "block" }} />
      </span>
      <span style={{ display: "flex", flexDirection: "column", gap: 9, minWidth: 0, flex: "1 1 auto" }}>
        <span style={{ fontFamily: MONO, fontSize: 14, color: "var(--text)" }}>{current.toUpperCase()}</span>
        <span style={{ fontSize: 12, color: "var(--dim)" }}>Drag the ring, or pick one below.</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {[0, 40, 90, 160, 215, 275].map((deg) => {
            const hex = hueHex(deg);
            return (
              <span
                key={deg}
                role="button"
                tabIndex={0}
                onClick={() => { setAngle(deg); onPick(hex); }}
                onKeyDown={(e) => { if (e.key === "Enter") { setAngle(deg); onPick(hex); } }}
                style={{ width: 28, height: 28, borderRadius: 999, background: hex, display: "block", cursor: "pointer", border: current.toLowerCase() === hex.toLowerCase() ? "2px solid var(--text)" : "2px solid transparent" }}
              />
            );
          })}
        </div>
        {value && (
          <span role="button" tabIndex={0} onClick={onReset} onKeyDown={(e) => { if (e.key === "Enter") onReset(); }}
            style={{ fontFamily: MONO, fontSize: 10.5, letterSpacing: "0.08em", color: "var(--amber-ink)", cursor: "pointer", alignSelf: "flex-start" }}>
            RESET TO LAPIS
          </span>
        )}
      </span>
...
// The field bodies, which are the same at both widths: only the chassis
// around them differs. The mobile frame puts the tabs in a scrolling chip
// row, the desktop frame puts them in a 232px rail, and both render this.
//
// `SettingsSections.jsx` is still the control set — nothing here adds,
// removes or renames one. This is layout.
export function SettingsBody({ section, sportsbooks = [], onOpenWheel }) {
          ...
          {section === "display" && (
            <>
              ...
              <Field label="Accent colour" hint="Used for anything selected or interactive — never for a result.">
                {onOpenWheel
                  ? <WheelRow value={d.accentColor} onOpen={onOpenWheel} onReset={() => setD("accentColor", null)} />
                  : <WheelInline value={d.accentColor} onPick={(hex) => setD("accentColor", hex)} onReset={() => setD("accentColor", null)} />}
              </Field>
              <Field
...
export const storageNoteFor = (section) => (section === "account"
...
export { Segmented, SelectRow, ToggleRow, InputRow, PaletteRows, Field, AccentSheet, WheelRow, WheelInline, SECTIONS, MONO, DISPLAY };
